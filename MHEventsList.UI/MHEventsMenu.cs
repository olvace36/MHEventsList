...
public class MHEventsMenu : IClickableMenu, IKeyboardSubscriber
{
	...
	public MHEventsMenu()
		: base((Game1.uiViewport.Width - 1280) / 2, (Game1.uiViewport.Height - 760) / 2, 1280, 760, true)
	{
	...
	public override void draw(SpriteBatch b)
	{
		...
		val5._002Ector(searchBox.X, searchBox.Y, searchBox.Width, searchBox.Height);
	...
	private void DrawNPCList(SpriteBatch b, int startY, int mouseX, int mouseY, Color textColor, bool darkMode)
	{
		...
		for (int num5 = 0; num5 < num4; num5++)
		{
			...
			val._002Ector(optionsPanelX, num3 + num5 * 28, optionsPanelWidth - 20, 26);
			if (num5 < npcSlots.Count)
			{
				npcSlots[num5].bounds = val;
			}
			bool flag = num7 == selectedNpcIndex;
			bool flag2 = val.Contains(mouseX, mouseY);
			if (flag)
			{
	...
	private void DrawAlternativeLayout(SpriteBatch b, int panelY, int panelHeight, int mouseX, int mouseY, Color textColor)
	{
		...
		bounds._002Ector(optionsPanelX + num8 + 5, num, num9, num8);
	...
	private void DrawOriginalLayout(SpriteBatch b, int panelY, int panelHeight, int mouseX, int mouseY, Color textColor)
	{
		...
		DrawSmallButton(b, heartsMinusBounds, "-", mouseX, mouseY, isActive: false, isClickable);
		Rectangle bounds = default(Rectangle);
		bounds._002Ector(optionsPanelX + num11 + 5, num10, num12, num11);
	...
	private void DrawBigButton(SpriteBatch b, Rectangle bounds, string text, int mouseX, int mouseY, bool isActive = false)
	{
		...
		bool flag = bounds.Contains(mouseX, mouseY);
		Color val;
		Color val2;
	...
	private void DrawSmallButton(SpriteBatch b, Rectangle bounds, string text, int mouseX, int mouseY, bool isActive = false, bool isClickable = true, float textScale = 0.9f)
	{
		...
		bool flag = isClickable && bounds.Contains(mouseX, mouseY);
		Color val;
		Color val2;
		if (MHEventsListMod.Config.UseDarkTheme)
		{
	...
	private void DrawEventEntry(SpriteBatch b, EventData evt, int yPos, int buttonIndex)
	{
		...
		if (eventListFormat == EventListFormat.ThreeLines)
		{
			...
			if (flag2)
			{
				...
				val4._002Ector((float)num + x, (float)num4);
		...
		{
			sourceRect._002Ector(310, 392, 16, 16);
			baseScale = 2.5f;
			val6 = Color.Gold;
		}
		else
		{
			...
			{
				sourceRect._002Ector(322, 498, 12, 12);
				baseScale = 2.8f;
	...
	private void DrawTooltips(SpriteBatch b)
	{
		...
		for (int i = 0; i < 6; i++)
		{
			...
			if (num < filteredEvents.Count)
			{
				...
				if (disabledMarkerAreas[i] != Rectangle.Empty && disabledMarkerAreas[i].Contains(mouseX, mouseY))
				{
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		Rectangle val = default(Rectangle);
		val._002Ector(searchBox.X, searchBox.Y, searchBox.Width, searchBox.Height);
		if (val.Contains(x, y))
		{
		...
		if (npcSearchBounds.Contains(x, y))
		{
			SelectNpcSearchBox();
			return;
		}
		DeselectNpcSearchBox();
		if (((ClickableComponent)scrollBar).containsPoint(x, y) || scrollBarRunner.Contains(x, y))
		{
			scrollBarHeld = true;
		...
		if (IsAndroid)
		{
			...
			val2._002Ector(eventBoxX, eventBoxY, eventBoxWidth, eventBoxHeight);
			if (val2.Contains(x, y))
			{
				bool flag = false;
				for (int i = 0; i < 6; i++)
				{
			...
			if (x >= optionsPanelX && x <= optionsPanelX + optionsPanelWidth && y >= base.yPositionOnScreen + 130 && y <= base.yPositionOnScreen + 130 + 256 && !npcSearchBounds.Contains(x, y))
			{
				bool flag2 = false;
		...
		if (sortOrderBounds.Contains(x, y) && sortBy > 0)
		{
			Game1.playSound("drumkit6", (int?)null);
		...
		if (heartsMinusBounds.Contains(x, y) && selectedNpcIndex >= 0)
		{
			Game1.playSound("drumkit6", (int?)null);
		...
		if (heartsPlusBounds.Contains(x, y) && selectedNpcIndex >= 0)
		{
			Game1.playSound("drumkit6", (int?)null);
		...
		if (clearFilterBounds.Contains(x, y))
		{
		...
		if (toggleSeenBounds.Contains(x, y))
		{
			Game1.playSound("drumkit6", (int?)null);
			if (filterMode == 0)
			{
		...
		if (toggleRelationshipsBounds.Contains(x, y))
		{
			Game1.playSound("drumkit6", (int?)null);
