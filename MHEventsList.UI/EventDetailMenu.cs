...
public class EventDetailMenu : IClickableMenu
{
	...
	public EventDetailMenu(EventData evt, IClickableMenu previousMenu = null)
		: base((((Rectangle)(ref Game1.uiViewport)).Width - 1030) / 2, (((Rectangle)(ref Game1.uiViewport)).Height - 680) / 2, 1030, 680, true)
	{
		eventData = evt;
	...
	private void DrawEventDetails(SpriteBatch b)
	{
		...
		if (!string.IsNullOrEmpty(eventData.RawPreconditions))
		{
			...
			for (int k = 0; k < array3.Length; k++)
			{
				...
				{
					((Rectangle)(ref item))._002Ector(num6, num13, contentWidth - 20, num10 - 2);
					string item2 = text11 + "\n\n" + text13;
	...
	private void DrawLegendPopup(SpriteBatch b)
	{
		...
		int num = 1000;
		int num2 = 700;
		int num3 = (((Rectangle)(ref Game1.uiViewport)).Width - num) / 2;
		int num4 = (((Rectangle)(ref Game1.uiViewport)).Height - num2) / 2;
		...
		((Rectangle)(ref val2))._002Ector(num3 + num - 48, num4 + 8, 36, 36);
		...
		((Rectangle)(ref scissorRectangle))._002Ector(legendScrollArea.X, legendScrollArea.Y, legendScrollArea.Width, legendScrollArea.Height);
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			((Rectangle)(ref item3))._002Ector(num13, num15, num / 2 - 40, num10);
		...
		for (int j = 0; j < array2.Length; j++)
		{
			...
			((Rectangle)(ref item4))._002Ector(num8, num18, num - 80, num10);
		...
		if (!((Rectangle)(ref legendScrollArea)).Contains(mouseX, mouseY))
		{
			return;
		}
		foreach (var item5 in list)
		{
			...
			if (((Rectangle)(ref val9)).Contains(mouseX, mouseY) && val9.Y >= num5 && val9.Y + val9.Height <= num5 + num6)
			{
	...
	private void DrawActionsPopup(SpriteBatch b)
	{
		...
		Rectangle scissorRectangle = default(Rectangle);
		((Rectangle)(ref scissorRectangle))._002Ector(actionsScrollArea.X, actionsScrollArea.Y, actionsScrollArea.Width, actionsScrollArea.Height);
		Rectangle scissorRectangle2 = ((GraphicsResource)b).GraphicsDevice.ScissorRectangle;
		RasterizerState rasterizerState = ((GraphicsResource)b).GraphicsDevice.RasterizerState;
		RasterizerState val5 = new RasterizerState
		{
	...
	private void DrawCPConditionsPopup(SpriteBatch b)
	{
		...
		int num7 = (((Rectangle)(ref Game1.uiViewport)).Width - num5) / 2;
		int num8 = (((Rectangle)(ref Game1.uiViewport)).Height - num6) / 2;
		...
		Rectangle val2 = default(Rectangle);
		((Rectangle)(ref val2))._002Ector(num7 + num5 - 48, num8 + 8, 36, 36);
		b.Draw(Game1.mouseCursors, val2, (Rectangle?)new Rectangle(337, 494, 12, 12), Color.White);
	...
	private void DrawTooltips(SpriteBatch b)
	{
		...
		foreach (Tuple<Rectangle, string> tooltipArea in tooltipAreas)
		{
			...
			if (((Rectangle)(ref item)).Contains(mouseX, mouseY))
			{
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		if (showingConditionDetail)
		{
			if (((Rectangle)(ref conditionDetailCloseBounds)).Contains(x, y))
			{
		...
		if (showingCPConditionsPopup)
		{
			if (((Rectangle)(ref cpConditionsCloseBounds)).Contains(x, y) || !((Rectangle)(ref cpConditionsPopupBounds)).Contains(x, y))
			{
				Game1.playSound("bigDeSelect", (int?)null);
		...
		if (cpConditionsClickArea != Rectangle.Empty && ((Rectangle)(ref cpConditionsClickArea)).Contains(x, y) && eventData.HasWhenConditions)
		{
		...
		foreach (Tuple<Rectangle, string, string, bool> conditionRow in conditionRows)
		{
			...
			if (((Rectangle)(ref item)).Contains(x, y))
			{
				Game1.playSound("bigSelect", (int?)null);
