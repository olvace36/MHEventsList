...
public sealed class EventOverlay : IClickableMenu
{
	...
	{
		if (_headerBounds.Contains(x, y))
		{
			return !_minimizeBounds.Contains(x, y);
		}
		return false;
	}
	...
	public void UpdatePosition()
	{
		...
		if (flag)
		{
			base.xPositionOnScreen = Game1.uiViewport.Width - ScaledWidth - 20 + overlayOffsetX;
		}
		else
		{
		...
		base.xPositionOnScreen = Math.Clamp(base.xPositionOnScreen, 0, Game1.uiViewport.Width - ScaledWidth);
		base.yPositionOnScreen = Math.Clamp(base.yPositionOnScreen, 0, Game1.uiViewport.Height - 200);
		CalculateLayout();
	}
	...
	private void CalculateLayout()
	{
		...
		base.xPositionOnScreen = Math.Clamp(base.xPositionOnScreen, 0, Game1.uiViewport.Width - base.width);
		base.yPositionOnScreen = Math.Clamp(base.yPositionOnScreen, 0, Game1.uiViewport.Height - base.height);
	...
	public override void draw(SpriteBatch b)
	{
		...
		bool flag = _minimizeBounds.Contains(mouseX, mouseY);
	...
	private void DrawPinnedEvent(SpriteBatch b, EventData evt, Rectangle bounds, Rectangle unpinBounds, bool darkMode, Theme theme, bool showDivider)
	{
		...
		Color val5 = (unpinBounds.Contains(mouseX, mouseY) ? theme.Error : theme.TextSecondary);
		float scale2 = Scale;
		...
		if (showDivider)
		{
			UIHelpers.DrawRect(b, new Rectangle(bounds.X + 5, bounds.Bottom + 3, bounds.Width - 10, 1), darkMode ? new Color(70, 70, 85) : new Color(170, 155, 125));
		}
	}
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		if (_minimizeBounds.Contains(x, y))
		{
		...
		{
			if (_headerBounds.Contains(x, y) || _isMinimized)
			{
				return;
			}
	...
	public override void releaseLeftClick(int x, int y)
	{
		if (_isDragging)
		{
			...
			{
				int num = Game1.uiViewport.Width - ScaledWidth - 20;
